...
const SearchBooks = async (
  ...
  try {
    ...
    if (searchBy === 'title') {
      ...
      countSearchResults = await db.query(`SELECT COUNT(*) FROM books LEFT JOIN genres ON books.genre_id = genres.genre_id WHERE books.title ILIKE '%' || $1 || '%' AND (genres.genre_name = $2 OR $2 IS NULL)`, [
        searchQuery,
        searchGenre,
      ])
