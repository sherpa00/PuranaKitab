...
const PlaceOrderOffline = async (cartsList: [{cartid: number}],userid: number,phoneNumber: number): Promise<ServiceResponse> => {
    try {
...
const PlaceOrderOnline = async (cartsList: [{cartid: number}], userid: number, phoneNumber: number, userEmail: string, cardDetails: OnlineCardDetails): Promise<ServiceResponse> => {
    try {
        let totalCartsAmount: number = 0
        const totalOrderedBooks: any = []
        // check if cart in cartsList exist for correct userid or not
        ...
        // update order with attached orere payemnt intent id
        const updatedOrderWithPayment = await db.query('UPDATE orders SET payment_intent_id = $1, payment_status = $2 WHERE orders.userid = $3 AND orders.orderid = $4 RETURNING *',[String(paymentIntent.id),'paid',userid,parseInt(addOrderStatus.rows[0].orderid)])
