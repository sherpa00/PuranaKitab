...
describe('Testing add user carts', () => {
  ...
  it('Should return success response when adding user carts', async () => {
    ;(db.query as jest.Mock).mockResolvedValueOnce(mockedGetBookDbQuery).mockResolvedValueOnce(mockedAddCartsDbQuery)
  ...
  it('Should return error response when database fails while adding user cart', async () => {
    ;(db.query as jest.Mock).mockResolvedValueOnce(mockedGetBookDbQuery).mockRejectedValue(new Error('Database Error'))
