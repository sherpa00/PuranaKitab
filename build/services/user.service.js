...
exports.DeleteUser = exports.UpdatePassword = exports.UpdateEmail = exports.UpdateUsername = exports.GetUserData = void 0;
const bcrypt_1 = require("bcrypt");
const db_configs_1 = require("../configs/db.configs");
const logger_utils_1 = __importDefault(require("../utils/logger.utils"));
// service for get  user's data
const GetUserData = (authenticatedUserid) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // get the user data from db
        const userData = yield db_configs_1.db.query('SELECT username,userid,email,role,last_logout,createat FROM users WHERE users.userid = $1', [authenticatedUserid]);
...
const UpdateUsername = (authenticatedUserId, newUsername) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const updateStatus = yield db_configs_1.db.query('UPDATE users SET username = $1 WHERE users.userid = $2 RETURNING userid,username,email', [newUsername, authenticatedUserId]);
...
const UpdateEmail = (authenticatedUserId, newEmail) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const updateStatus = yield db_configs_1.db.query('UPDATE users SET email = $1 WHERE users.userid = $2 RETURNING userid,username,email', [newEmail, authenticatedUserId]);
        if (updateStatus.rowCount <= 0) {
            return {
                success: false,
...
const UpdatePassword = (authenticatedUserId, oldPassword, newPassword) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // get original password hash from db
        const originalUserPassData = yield db_configs_1.db.query('SELECT password,salt FROM users WHERE users.userid = $1', [
            authenticatedUserId
        ]);
        ...
        const updateStatus = yield db_configs_1.db.query('UPDATE users SET password = $1 WHERE users.userid = $2 RETURNING userid,username,email', [newHashedPassword, authenticatedUserId]);
        if (updateStatus.rowCount <= 0) {
            return {
                success: false,
                message: 'Error while updating password'
...
const DeleteUser = (authenticatedUserId, password) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // first get the user for db
        const originalUserPassData = yield db_configs_1.db.query('SELECT userid,email,username,password FROM users WHERE users.userid = $1', [authenticatedUserId]);
        if (originalUserPassData.rowCount <= 0) {
            return {
                success: false,
        ...
        const deleteStatus = yield db_configs_1.db.query('DELETE FROM users WHERE users.userid = $1', [authenticatedUserId]);
