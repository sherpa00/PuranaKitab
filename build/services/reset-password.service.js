...
const ResetPassword = (resetToken, newPassword) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const foundResetToken = yield db_configs_1.db.query('SELECT * FROM reset_tokens WHERE reset_tokens.token = $1 AND reset_tokens.expiry_date > NOW()', [resetToken]);
        ...
        const foundUser = yield db_configs_1.db.query('SELECT * FROM users WHERE users.email = $1', [foundResetToken.rows[0].email]);
        ...
        // update user db
        const updateUserPassword = yield db_configs_1.db.query('UPDATE users SET password = $1 WHERE users.email = $2 RETURNING *', [
            newHashedPassword,
        ...
        const removeResetToken = yield db_configs_1.db.query('DELETE FROM reset_tokens WHERE reset_tokens.token = $1 AND reset_tokens.email = $2 RETURNING *', [resetToken, foundUser.rows[0].email]);
