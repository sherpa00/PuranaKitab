...
const GetAllReviews = (bookID) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const foundBook = yield db_configs_1.db.query('SELECT bookid FROM books WHERE books.bookid = $1', [bookID]);
        ...
        logger_utils_1.default.error(err, 'Error while getting all book reviews');
        return {
            success: false,
            message: 'Failed to get all book reviews'
        };
...
const AddReview = (userID, bookID, userName, reviewStars, reviewMessage) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const foundBook = yield db_configs_1.db.query('SELECT bookid FROM books WHERE bookid = $1', [bookID]);
        if (foundBook.rowCount <= 0) {
            return {
                success: false,
                message: 'No Book Found'
        ...
        const reviewsFound = yield db_configs_1.db.query('SELECT reviewid FROM reviews WHERE userid = $1 AND bookid = $2', [
            userID,
            bookID
        ]);
        ...
        logger_utils_1.default.error(err, 'Error while adding book review');
        return {
            success: false,
            message: 'Failed to add book review'
        };
...
const RemoveSinlgeReview = (reviewID) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const foundReview = yield db_configs_1.db.query('SELECT reviewid FROM reviews WHERE reviews.reviewid = $1', [reviewID]);
        ...
        logger_utils_1.default.error(err, 'Error while removing a book review');
        return {
            success: false,
            message: 'Failed to remove book review'
        };
...
const RemoveAllReviews = (bookID) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // verify if book exits or not
        const bookFound = yield db_configs_1.db.query('SELECT bookid FROM books WHERE books.bookid = $1', [bookID]);
        ...
        logger_utils_1.default.error(err, 'Error while removing all book reviews');
        return {
            success: false,
            message: 'Failed to remove all book reviews'
        };
