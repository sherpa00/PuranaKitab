...
exports.DeleteBookImage = exports.UpdateBookImg = exports.AddBookImg = exports.RemoveBookWithId = exports.UpdateBook = exports.AddBook = exports.GetOnlyOneBook = exports.GetAllBooks = void 0;
...
const AddBook = (bookData, authorFirstname, authorLastname) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const getAuthorStatus = yield db_configs_1.db.query('SELECT * FROM authors WHERE firstname = $1 AND lastname = $2', [
            authorFirstname,
            authorLastname
        ]);
        ...
        if (getAuthorStatus.rowCount <= 0) {
            ...
            const addAuthorStatus = yield db_configs_1.db.query('INSERT INTO authors (firstname, lastname) VALUES ($1, $2) RETURNING *', [
                authorFirstname,
                authorLastname
            ]);
        ...
        const addBookStatus = yield db_configs_1.db.query('INSERT INTO books (authorid, title, price, publication_date, book_type, book_condition, available_quantity, isbn, description) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *', [
            currentAuthorid,
            bookData.title,
            bookData.price,
...
const RemoveBookWithId = (bookID) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (bookImagesWithId.rowCount === 2) {
            ...
            deleteBookImagesWithIdStatus = yield db_configs_1.db.query('DELETE FROM book_images WHERE bookid = $1 RETURNING *', [bookID]);
        ...
        if (foundBookReviews.rowCount > 0) {
            ...
            const deleteBookReviews = yield db_configs_1.db.query('DELETE FROM reviews WHERE reviews.bookid = $1 RETURNING *', [bookID]);
        ...
        if (foundCarts.rowCount > 0) {
            ...
            const deleteCarts = yield db_configs_1.db.query('DELETE FROM carts WHERE carts.bookid = $1 RETURNING *', [bookID]);
...
const AddBookImg = (bookid, imgPath, imgType) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const isBookImgFound = yield db_configs_1.db.query('SELECT * FROM book_images WHERE bookid = $1 AND img_type = $2', [
            bookid,
            imgType
        ]);
        ...
        const imgToDbStatus = yield db_configs_1.db.query('INSERT INTO book_images(bookid, img_src, img_type, img_public_id) VALUES ($1, $2, $3, $4) RETURNING *', [bookid, imgUploadStatus.imgURL, imgType, imgUploadStatus.imgPublicId]);
...
const DeleteBookImage = (bookid, imgType) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const bookImgRemoveStatus = yield db_configs_1.db.query('DELETE FROM book_images WHERE bookid = $1 AND img_type = $2', [
            bookid,
            imgType
        ]);
