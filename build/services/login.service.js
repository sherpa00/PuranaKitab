...
const LoginUser = (userInfo) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const isVerified = yield (0, bcrypt_1.compare)(userInfo.password, foundUser.rows[0].password);
        if (!isVerified) {
            return {
                success: false,
                message: 'User Email or Password incorrect'
        ...
        const token = (0, jsonwebtoken_1.sign)({ sub: foundUser.rows[0].userid, subRole: foundUser.rows[0].role, subPass: foundUser.rows[0].password }, PRIVATE_KEY, {
            expiresIn: '1h',
            algorithm: 'RS256'
        });
        return {
        ...
        logger_utils_1.default.error(err, 'Error while login in');
        return {
            success: false,
            message: 'User Email or Password incorrect'
        };
